...
export async function createWallet(
  ...
  const walletConfig = {
    ...
    indexerClientConnection: {
      ...
      // Limit the number of events waiting in memory while the wallet
      // catches up with the indexer.
      bufferSize: 2000,

      // Reopen the WebSocket subscription after the queue has drained.
      resumeThreshold: 200,
    },

    // Process events in larger batches during initial synchronization.
    batchUpdates: {
      size: 100,
      timeout: 10,
      spacing: 0,
    ...
    relayURL: new URL(
      opts.networkConfig.node.replace(/^http/, 'ws'),
    ),
  ...
  const wallet = await WalletFacade.init({
    ...
    shielded: async (config) => {
      ...
      if (saved.shielded !== undefined && saved.shielded !== null) {
        try {
          const restoredWallet = cls.restore(
            String(saved.shielded),
          );
...
export async function persistWalletState(
  ...
  for (const kind of CHILD_KINDS) {
    try {
      ...
      process.stderr.write(
        `  ⚠ Could not serialize ${kind} wallet state (${msg}); next run will resync this wallet.\n`,
      );
    }
  }
